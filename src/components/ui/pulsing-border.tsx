...
vec3 shade(vec2 uv, vec2 p, float t) {
  // The generator hardcodes box = vec2(0.82, 0.47), a fixed rectangle that
  // ignores the canvas: on a wide viewport it lands in the middle of the
  // frame and cuts across whatever sits there. Deriving the half-extents
  // from the resolution makes it hug the canvas edge at any aspect ratio,
  // so it reads as an outline instead of a shape drawn over the content.
  vec2 halfExtent = 0.5 * u_resolution.xy / min(u_resolution.x, u_resolution.y);
  vec2 box = (halfExtent - vec2(0.01)) * u_scale;
  vec2 d = abs(p) - box;
  float outside = length(max(d, 0.0)) + min(max(d.x, d.y), 0.0);
  float thickness = mix(0.004, 0.11, u_paramA);
  ...
  // Floor the trail so the outline is a continuous hairline with a pulse
  // travelling along it. At trail = 0 the generator returns colours[0] —
  // the ground colour — which on a matching background makes the border
  // disappear between pulses and read as blips rather than an outline.
  float lit = 0.16 + 0.84 * trail;
  float innerGlow = exp(-abs(outside) * 60.0) * 0.20;
  return mix(u_colors[0], palette(lit), clamp(edge + innerGlow, 0.0, 1.0));
}
...
// Retuned from the generator's default blue/cyan recipe to the portfolio's
// own palette: near-black ground, warm bronze, the gold accent, warm white.
// The default cyan fought both the gold accent and the per-fold colours.
const UNIFORMS = {
  colors: [
    [0.039215686274509803, 0.039215686274509803, 0.047058823529411764],
    [0.2901960784313726, 0.22745098039215686, 0.10980392156862745],
    [0.8941176470588236, 0.7019607843137254, 0.38823529411764707],
    [0.9803921568627451, 0.9647058823529412, 0.9372549019607843],
    [0.9803921568627451, 0.9647058823529412, 0.9372549019607843],
    [0.9803921568627451, 0.9647058823529412, 0.9372549019607843],
    [0.9803921568627451, 0.9647058823529412, 0.9372549019607843],
    [0.9803921568627451, 0.9647058823529412, 0.9372549019607843],
  ...
  scale: 1.000,
  intensity: 0.350,
  paramA: 0.060,   // a hairline outline, ~6px on a 640px-tall canvas
  warp: 0.000,
  detail: 1.824,
  contrast: 1.005,
  brightness: 0.000,
...
export function ShaderBackground({
  className,
  // Render cost knobs. Defaults match the generator's original output; a
  // decorative background behind text does not need a retina-resolution
  // full-screen fill every frame, so callers can dial both down.
  maxDpr = 2,
  ...
  useEffect(() => {
    ...
    // The generator's version discards compile/link status, so a bad shader
    // fails silently as an empty canvas with nothing in the console.
    const compile = (type: number, src: string) => {
      ...
      if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
        console.error(
          `[ShaderBackground] ${type === gl.VERTEX_SHADER ? "vertex" : "fragment"} shader failed to compile:`,
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error("[ShaderBackground] program failed to link:", gl.getProgramInfoLog(program))
    }
